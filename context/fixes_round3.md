# CONTEXT FILE: Fix Round 3 - Future attached to different loop
# Root cause: SQLAlchemy async engine created once but used across
# multiple event loops created by run_async(). Each run_async() call
# creates a new loop, but the engine's connection pool is tied to the
# original loop. Fix: create engine fresh inside every async function.

---

## Replace analysis_task.py Entirely
...
[ ] Replace analysis_task.py with content above
[ ] Rebuild: docker-compose down && docker-compose up --build -d
[ ] Submit a repo URL
[ ] Worker logs show step-by-step progress (no Future/loop errors)
[ ] Analysis completes successfully
[ ] Results appear in frontend

Key change: NullPool on every engine means no connections are
held between event loops. Each run_async() is fully isolated.
